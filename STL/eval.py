...
def evaluate_f_(phi, sig, time, t=0, points=500, time_period=20):
    p = re.sub(r"(\d)+(\.\d)?,((\d)+(\.\d)?|inf)", "", str(phi))
    p = p.strip("O").strip("[").strip("]").strip("~").strip("(") \
        .strip(")").replace(" ", "")
    p = p.split("<")
    a, b = phi.interval
    if t - b == -float('inf'):
        ...
        prev_ab = sig[p[0]][int(t - b * points / time_period):
                            int(t - a * points / time_period)]
    if len(prev_ab) <= 0:
        return evaluate(phi.arg, sig, time, t)
    m = -float('inf')
    for j, n in enumerate(prev_ab):
        if t - b * int(points / time_period) < 0:
            ...
            temp = evaluate(phi.arg, sig, time,
                            int(t - b * points / time_period + j))
        if temp > m:
            m = temp
    return m
...
def evaluate_until(phi, sig, time, t=0, points=500, time_period=20):
    ...
    for j in range(len(time) - 2, t - 1, -1):
...
@evaluate.register(ast.Since)
def evaluate_since(phi, sig, time, t=0, points=500, time_period=20):
    temp1 = evaluate(phi.arg1, sig, time, len(time) - 1)
    temp2 = evaluate(phi.arg2, sig, time, len(time) - 1)
    y_k_1 = min(temp1, temp2)
    for j in range(len(time) - 2, t - 1, -1):
        temp1 = evaluate(phi.arg1, sig, time, j)
        temp2 = evaluate(phi.arg2, sig, time, j)
        y_k_1 = max(min(temp1, temp2), min(temp2, y_k_1))
    return y_k_1
